...
(defgroup shu-git nil
  "Miscellaneous helper functions for using git."
  :group 'applications)

(defcustom shu-git-signoff nil
  "If true, then any commit generated by SHU-GIT-INSERT-GIT-COMMIT includes the
-s option to add the phrase \"Signed-off-by:\" to the commit message.  Some open
source repositories require the sign-off message in every commit and this can be
a useful way to remember to do add it."
  :type 'boolean
  :group 'shu-git)
...
in a file called \"why.txt\".  if the custom variable SHU-GIT-SIGNOFF is true,
then the -s option is added to the commit command."
  (interactive)
  (let ((s-option (if shu-git-signoff " -s" "")))
  (insert (concat "git commit" s-option " -F why.txt"))
    ))
