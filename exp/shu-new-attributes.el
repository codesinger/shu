;;; shu-new-attributes.el --- Shu project code for dealing wth C++ in Emacs
;;
...
;;  shu-make-new-class
;;
(defun shu-make-new-class (start end)
  "Doc string."
  (interactive "r")
  (let (
        (class-list)
...
of nullable values.  Also, generate the declaration for the bindValues
function and all of the indivual bind functions, one per column."
  (let ((attrs attributes)
        (attr-info)
        (name)
        (uname)
    ...
    (while attrs
      ...
      (when comment
        (insert
         (concat "\n"
                 ipad "/*!\n"
                 ipad " * \\brief Bind the value for " name "\n"
                 ipad " */\n")))
...
(defun shu-cpp-attributes-gen-bind-values-gen (class-name attributes)
  "Generate the individual functions that bind the values for each column."
  (let ((attrs attributes)
        (attr-info)
        (name)
        (funame)
    ...
    (while attrs
      ...
      (shu-cpp-extract-attr-info attr-info name data-type full-data-type comment
                                 reference nullable column-name column-count
                                 enum-base reset-value)
      (setq funame (concat "bind" (shu-upcase-first-letter name)))
      (insert
       (concat
        ipad funame "(binder, file, line);\n"
        ))
      (setq attrs (cdr attrs)))
    (insert "}\n")
    (shu-cpp-attributes-gen-bind-values-functions class-name attributes)
    ))
...
;;
;;  shu-cpp-attributes-gen-bind-values-functions
;;
(defun shu-cpp-attributes-gen-bind-values-functions (class-name attributes)
  "Generate the individual functions that bind the values for each column."
  (let ((attrs attributes)
        (attr-info)
        (name)
      ...
      (if contained-class
        ...
        (if (not nullable)
          ...
          (insert
           (concat
            ipad  "if ( !" uname " )\n"
            ipad ipad "binder.bindNull(" column-name ", file, line);\n"
            ipad  "else\n"
            bind-prefix)))
...
;;; shu-new-attributes.el ends here
